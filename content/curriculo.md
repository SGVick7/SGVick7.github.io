---
title: "Currículo"
date: 2026-06-18
---
# Ana Victória Silva Gonçalves

Estudante de Estatística com interesse em Ciência de Dados, Análise Estatística, Visualização de Dados e Pesquisa Quantitativa.

## Contato

* E-mail: [ana.s.goncalves@ufv.br](mailto:seuemail@exemplo.com)
* GitHub: https://github.com/SGVick7
* LinkedIn: https://linkedin.com/in/seu-perfil
* Viçosa - MG

---

## Formação Acadêmica

**Bacharelado em Ciência de Dados**
Universidade Federal de Viçosa (UFV)

Previsão de conclusão: 2030

---

## Áreas de Interesse

* Ciência de Dados
* Programação em Python
* Estatística Aplicada
* Análise Exploratória de Dados
* Inferência Estatística
* Aprendizado de Máquina
* Visualização de Dados
* Pesquisa Quantitativa

---

## Competências Técnicas

### Linguagens

* R
* Python

### Ferramentas

* RStudio
* Git e GitHub
* Python
* Excel

### Bibliotecas

* ggplot2
* dplyr
* tidyr
* numpy
* pandas
* matplotlib

---

## Projetos

### Análise Estatística de Dados Agrícolas

* Organização e limpeza de dados.
* Construção de gráficos exploratórios.
* Aplicação de medidas descritivas.
* Interpretação de resultados estatísticos.

### Visualização de Dados com R

* Desenvolvimento de gráficos utilizando ggplot2.
* Comparação de indicadores através de visualizações estatísticas.
* Produção de relatórios em R Markdown.

### Portfólio de Estatística

* Desenvolvimento de site pessoal utilizando Hugo e GitHub Pages.
* Publicação de projetos acadêmicos e estudos estatísticos.
* Versionamento utilizando Git.

---

## Conhecimentos Estatísticos

* Estatística Descritiva
* Probabilidade
* Amostragem
* Testes de Hipóteses
* Intervalos de Confiança
* Regressão Linear
* Análise Exploratória de Dados

---

## Idiomas

* Português: Nativo
* Inglês: Leitura técnica




